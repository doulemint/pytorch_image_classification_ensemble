...
class albu_transforms_train:
    def __init__(self,configs:yacs.config.CfgNode):
    ...
    def __call__(self) -> list:
        return  [
                ...
                A.RandomResizedCrop(height=self.configs.input_size[1], width=self.configs.input_size[0], scale=(0.8, 1.0), p=1),
                A.CenterCrop(height=self.configs.input_size[1], width=self.configs.input_size[0]),
                A.Transpose(p=0.5),
                A.HorizontalFlip(p=0.5),
                A.VerticalFlip(p=0.5),
...
class albu_transforms_train_cutmix:
...
class albu_transforms_val:
    def __init__(self,config:yacs.config.CfgNode):
        self.configs = config
    def __call__(self) -> list:
        return[
                A.Resize(height=self.configs.input_size[1], width=self.configs.input_size[0]),  # clw note: CenterCrop not good
                A.Normalize(),   #A.Normalize(mean=(0.43032, 0.49673, 0.31342), std=(0.237595, 0.240453, 0.228265)),
                ToTensorV2(),
                ######## HolyCHen Vit !!!
...
class agu:
    def __init__(self,configs:yacs.config.CfgNode):
        temp_aug  = albu_transforms_train(configs)
        self.train_aug = A.Compose(temp_aug())
        temp_aug = albu_transforms_train_cutmix(configs)
        self.train_aug_cutmix = A.Compose(temp_aug())
        temp_aug = albu_transforms_val(configs)
        self.val_aug = A.Compose(temp_aug())


class CassavaTrainingDataset(Dataset):
    # define dataset
    def __init__(self,label_list,configs, mode="train"):
        super(CassavaTrainingDataset,self).__init__()
        self.label_list = label_list
        self.mode = mode
        self.agu = agu(configs)
        ...
        if self.mode == "train" or self.mode == "val":  # train or val, all need label
            ...
            if self.mode == "train":
                ...
                if random.random() < self.do_cutmix_prob:
                    ...
                    train_aug = self.agu.train_aug
                    img = train_aug(image=img)['image']  # clw note: 考虑到这里有crop等导致输入尺寸不同的操作，把resize放在后边
                    label = torch.zeros(self.configs.num_classes).scatter_(0, label, 1)
            elif self.mode == "val":
            ...
            img = cv2.imread(filename)
            input_size = self.configs.input_size if isinstance(self.configs.input_size, tuple) else (configs.input_size, configs.input_size)
    ...
    def do_mixup(self, img, label, index):
        ...
        '''
        input_size = self.configs.input_size if isinstance(self.configs.input_size, tuple) else (self.configs.input_size, self.configs.input_size)
        mixup_ratio = np.random.beta(1.5, 1.5)

        train_aug = A.Compose(albu_transforms_train(self.configs))
        img = train_aug(image=img)['image']  # clw note: 考虑到这里有crop等导致输入尺寸不同的操作，把resize放在后边

        r_idx = random.choice(np.delete(np.arange(len(self.imgs)), index))
        r_filename, r_label = self.imgs[r_idx]
        r_img = cv2.imread(os.path.join(self.configs.dataset + "/train/", r_filename))
        r_img = cv2.resize(r_img, input_size)
        r_img = cv2.cvtColor(r_img, cv2.COLOR_BGR2RGB)
        r_img = train_aug(image=r_img)['image']
        img_new = img * mixup_ratio + r_img * (1 - mixup_ratio)
        label_one_hot = torch.zeros(self.configs.num_classes).scatter_(0, label, 1)
        r_label = torch.tensor(r_label).long()
        r_label_one_hot = torch.zeros(self.configs.num_classes).scatter_(0, r_label, 1)
    ...
    def do_cutmix(self, img, label, index):
        ...
        train_aug = A.Compose(albu_transforms_train(self.configs))
        train_aug_cutmix = self.agu.train_aug_cutmix
...
class CassavaValDataset(Dataset):  # evaluate才使用,训练不用
    # define dataset
    def __init__(self, label_list,configs):
...
class TrainDataset(Dataset):
    def __init__(self, df,configs, mode="train"):
        self.df = df
        self.file_names = df['image_id'].values
        self.labels = df['label'].values
        self.mode = mode
        self.configs=configs
        self.aug = agu(configs)
    ...
    def __getitem__(self, idx):
        ...
        img = cv2.imread( os.path.join(self.configs.dataset_merge_csv, self.file_names[idx]) )
        input_size = self.configs.input_size if isinstance(self.configs.input_size, tuple) else (self.configs.input_size, self.configs.input_size)
        img = cv2.resize(img, input_size)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        train_aug = A.Compose(albu_transforms_train(self.configs))
        val_aug = self.aug.val_aug
        ...
        if self.mode == "train":
            ...
            if random.random() < 0.5:
                ...
                r_img = cv2.imread( os.path.join(self.configs.dataset_merge_csv, self.file_names[r_idx]))
                r_img = cv2.resize(r_img, input_size)
                r_img = cv2.cvtColor(r_img, cv2.COLOR_BGR2RGB)
                r_img = train_aug(image=r_img)['image']
